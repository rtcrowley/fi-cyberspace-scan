...
def settings():
    ...
    print bco.BLU+brk
    print bco.RED+"DO NOT USE AGAINST UNAUTHORIZED INTRUSION COUNTERMEASURES ELECTRONICS"+bco.ENC
  ...
  for i in modelist:
      ...
      if tmode < 1:
        ...
        for j in ko:
            ...
            x = len(xeq.content)
            #print x
            # Evaluates each path in a directory by reflected bytes.
            # x is the current path/file size, matched against known invalid size -- beval.
            if deepspace is True and x != beval:
                 print bco.BEG + "     --Path: "+bco.ENC ,j
            if deepspace is False and x != beval:
                 print bco.BEG + "     --Path: "+bco.ENC ,j
              #,bco.BLU + "bytes:"+bco.ENC,x
